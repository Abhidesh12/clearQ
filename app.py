...
def url_for(request: Request, endpoint: str, **kwargs) -> str:
    """
    ...
    try:
        ...
        # Fallback routes for templates that aren't registered as FastAPI endpoints
        routes = {
            'home': '/',
            'dashboard': '/dashboard',
...
templates.env.globals.update({
    "url_for": lambda endpoint, **kwargs: f"/{endpoint}",  # Will be overridden in each route
...
async def index(request: Request, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_from_cookie)):
    ...
    return templates.TemplateResponse("index.html", {
        ...
        "now": datetime.now(),
        "url_for": lambda endpoint, **kwargs: url_for(request, endpoint, **kwargs)
    })
...
async def enroll_page(request: Request, current_user: User = Depends(get_current_user_from_cookie)):
    if not current_user:
        return RedirectResponse(url="/login", status_code=303)
...
async def mentor_profile(request: Request, username: str, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_from_cookie)):
...
async def service_detail(request: Request, service_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user_from_cookie)):
...
async def update_availability_api(
    request: Request,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user_from_cookie),
...
async def register_page(request: Request, current_user: User = Depends(get_current_user_from_cookie)):
    if current_user:
        return RedirectResponse(url="/dashboard", status_code=303)
...
async def create_booking(
    ...
    scheduled_datetime = datetime.strptime(f"{scheduled_for} {time_slot}", "%Y-%m-%d %H:%M")
    ...
    mentor_user = db.query(User).filter(User.id == mentor.user_id).first()
    if mentor_user:
        order_data['notes']['mentor'] = mentor_user.full_name
...
async def terms_page(request: Request, current_user: User = Depends(get_current_user_from_cookie)):
...
async def privacy_page(request: Request, current_user: User = Depends(get_current_user_from_cookie)):
...
async def mentorship_program(request: Request, current_user: User = Depends(get_current_user_from_cookie)):
